    ...
    def _evaluate_acc_f1(self):
        ...
        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(self.test_data_loader):
                t_inputs = [t_sample_batched[col].to(opt.device) if col != 'context'  else t_sample_batched[col] for col in self.opt.inputs_cols  ]
        ...
        precision_macro = metrics.precision_score(t_targets_all.cpu(),torch.argmax(t_outputs_all, -1).cpu(), labels=[0, 1], average='macro')
        recall_macro = metrics.recall_score(t_targets_all.cpu(),torch.argmax(t_outputs_all, -1).cpu(), labels=[0, 1], average='macro')
...
if __name__ == '__main__':
    ...
    input_colses = {
        'affgcn': ['context_indices',  'dependency_graph', 'sentic_graph'],
    }
    initializers = {
